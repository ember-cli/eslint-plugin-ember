...
const ALLOWED_RULESSET_FOR_TEMPLATES = new Set(['no-undef', 'ember/ember-template-lint']);
...
function mapRange(messages, filename) {
  ...
  return filtered.map((message) => {
    if (message.ruleId === 'ember/ember-template-lint') {
      message.message = message.message.replace('(ember/ember-template-lint)', '');
      return message;
    }
