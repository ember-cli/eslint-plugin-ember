...
module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Require explicit this for property access in templates',
    ...
    schema: [],
    messages: {
      noImplicitThis: 'Ambiguous path "{{path}}" is not allowed. Use "@{{path}}" if it is a named argument or "this.{{path}}" if it is a property on the component.',
    },
  },
  ...
  create(context) {
    return {
      'GlimmerPathExpression'(node) {
        ...
        if (path.startsWith('@') || path.startsWith('this.')) return;
        
        // Skip built-in helpers and keywords
        const builtIns = [
          'yield', 'outlet', 'has-block', 'has-block-params',
          'if', 'unless', 'each', 'let', 'with', 'each-in',
          'concat', 'get', 'array', 'hash', 'log', 'debugger',
          'component', 'helper', 'modifier', 'mount',
        ];
        ...
        if (node.parent && node.parent.type === 'GlimmerMustacheStatement') {
          ...
          if (node.parent.params && node.parent.params.length > 0) return;
          // If it has hash pairs, it's likely a helper
          if (node.parent.hash && node.parent.hash.pairs && node.parent.hash.pairs.length > 0) return;
        }
        ...
        if (node.parent && node.parent.type === 'GlimmerBlockStatement') {
          ...
          if (blockParams.includes(path.split('.')[0])) return;
        }
        ...
        if (!path.includes('.') || !path.startsWith('this.')) {
          ...
          if (firstPart[0] === firstPart[0].toUpperCase()) return;
