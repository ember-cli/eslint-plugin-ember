...
module.exports = {
  ...
  create(context) {
    ...
    function checkElementOrder(node) {
      ...
      const hasSplat = tokenGroups[TokenType.SPLATTRIBUTES]?.length > 0;
      const splatPos = hasSplat ? getNodePosition(tokenGroups[TokenType.SPLATTRIBUTES][0]) : -1;

      // When splattributes is present and surrounded by other items,
      // split items into before/after splat and check each side independently.
      function getGroupSets() {
        ...
        const beforeSplat = {};
        const afterSplat = {};
        for (const [type, items] of Object.entries(tokenGroups)) {
          if (type === TokenType.SPLATTRIBUTES || type === TokenType.COMMENTS) {
            beforeSplat[type] = [];
            afterSplat[type] = [];
            continue;
          }
          beforeSplat[type] = (items || []).filter((n) => getNodePosition(n) < splatPos);
          afterSplat[type] = (items || []).filter((n) => getNodePosition(n) > splatPos);
        }
        return [beforeSplat, afterSplat];
      }

      const groupSets = getGroupSets();

      // Check group ordering for each side of splattributes
      for (const groups of groupSets) {
        const maxPosPerGroup = {};
        const minPosPerGroup = {};
        for (const tokenType of appliedOrder) {
          const items = groups[tokenType];
        ...
        const activeTypes = appliedOrder.filter((t) => groups[t] && groups[t].length > 0);
        ...
        for (let i = 0; i < activeTypes.length; i++) {
          for (let j = i + 1; j < activeTypes.length; j++) {
            ...
            if (maxPosPerGroup[currentType] > minPosPerGroup[laterType]) {
              const offendingItem = groups[currentType].find(
      ...
      // Check alphabetization within groups (also split by splattributes)
      if (alphabetize) {
        for (const groups of groupSets) {
          const activeTypes = appliedOrder.filter((t) => groups[t] && groups[t].length > 0);
