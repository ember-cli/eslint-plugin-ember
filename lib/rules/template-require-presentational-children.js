...
function findSemanticDescendant(children, nonSemanticTags) {
  for (const child of children || []) {
    if (child.type === 'GlimmerElementNode') {
      if (
        SEMANTIC_ELEMENTS.has(child.tag) &&
        !nonSemanticTags.has(child.tag) &&
        !hasPresentationalRole(child)
      ) {
        return child;
      }
      const found = findSemanticDescendant(child.children, nonSemanticTags);
      if (found) {
        return found;
      }
    }
...
module.exports = {
  ...
  create(context) {
    ...
    const nonSemanticTags = new Set(options.additionalNonSemanticTags || []);
    ...
    return {
      GlimmerElementNode(node) {
        ...
        if (ROLES_REQUIRING_PRESENTATIONAL_CHILDREN.has(role)) {
          const semanticChild = findSemanticDescendant(node.children, nonSemanticTags);
          if (semanticChild) {
            context.report({
              node,
              messageId: 'invalid',
