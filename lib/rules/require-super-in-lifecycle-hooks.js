...
module.exports = {
  ...
  create(context) {
    ...
    function checkAndReport(
      ...
      const hasSuperMatchingNodeResult = hasMatchingNode(body, (bodyChild) =>
        isNativeClass ? isNativeSuper(bodyChild, hookName) : isClassicSuper(bodyChild)
      );
      const hasSuper = hasSuperMatchingNodeResult.foundMatch;

      if (hookName === 'init' && hasSuper && requireArgs) {
        const superNode = hasSuperMatchingNodeResult.foundNode;
        const hasSuperWithArguments = superNode.arguments && superNode.arguments.length > 0;
