    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (node.callee.name === 'get' && types.isThisExpression(node.arguments[0])) {
          ...
          delete unusedServicesMapping[name];
        } else if (node.callee.name === 'getProperties' && types.isThisExpression(node.arguments[0])) {
          // 2. getProperties(this, [..., 'foo..', ...]); or getProperties(this, ..., 'foo..', ...);
          const argArray = (types.isArrayExpression(node.arguments[1])) ? node.arguments[1].elements : node.arguments.slice(1);
          argArray.forEach((elem) => {
          ...
          delete unusedServicesMapping[name];
        } else if (types.isThisExpression(node.callee.object) && node.callee.property.name === 'getProperties') {
          // 4. this.getProperties([..., 'foo..', ...]); or this.getProperties(..., 'foo..', ...);
          const argArray = (types.isArrayExpression(node.arguments[0])) ? node.arguments[0].elements : node.arguments;
          argArray.forEach((elem) => {
            const name = splitValue(elem.value);
            delete unusedServicesMapping[name];
          });
      ...
      'Program:exit': () => {
        Object.keys(unusedServicesMapping).forEach((name) => {
