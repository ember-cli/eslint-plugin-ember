...
function getErrorMessage(serviceName) {
  return `The service \`${serviceName}\` is not referenced in this JS file, but it may still be referenced in the corresponding HBS file. Please remove this service injection if it is not being used within the HBS file.`;
}
    ...
    return {
      ...
      CallExpression(node) {
        if (emberUtils.isAnyEmberCoreModule(context, node)) {
          ...
          if (types.isIdentifier(node.callee) && types.isThisExpression(node.arguments[0])) {
            if (node.callee.name === 'get') {
              // 1. get(this, 'foo...');
              const name = splitValue(node.arguments[1].value);
              currentClass.uses.add(name);
            } else if (node.callee.name === 'getProperties') {
      ...
      MemberExpression(node) {
        const currentClass = classStack.peek();
        if (currentClass && propertyGetterUtils.isSimpleThisExpression(node)) {
