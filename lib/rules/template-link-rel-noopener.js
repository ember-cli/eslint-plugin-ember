      ...
      GlimmerElementNode(node) {
        ...
        if (!hasProperRel) {
          context.report({
            ...
            fix:
              relAttr && relAttr.value?.type !== 'GlimmerTextNode'
                ? undefined
                : (fixer) => {
                    if (relAttr) {
                      // Existing rel with text value — clean up and add noopener noreferrer
                      const oldValue = relAttr.value.chars
                        .trim()
                        .replaceAll(/\s+/g, ' ')
                        .replaceAll(/\b(noopener|noreferrer)\b/g, '')
                        .trim();
                      const newValue = oldValue
                        ? `${oldValue} noopener noreferrer`
                        : 'noopener noreferrer';
                      return fixer.replaceText(relAttr.value, `"${newValue}"`);
                    }
                    // No rel attribute — insert one
