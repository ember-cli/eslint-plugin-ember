...
module.exports = {
  ...
  create(context) {
    ...
    const isAClassicLifecycleHook = function(keyName) {
      return isComponentLifecycleHookName(keyName) && isAComponent;
    };

    return {
      ClassDeclaration(node) {
        isAComponent = node.superClass && node.superClass.name === 'Component';
      },

      MemberExpression(node) {
        isAComponent = node.object.name === 'Component';
      },
