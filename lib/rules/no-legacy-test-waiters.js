...
module.exports = {
  ...
  create(context) {
    let testWaitersImportFound = false;
    ...
    return {
      ImportDeclaration(node) {
        ...
        node.specifiers.forEach(specifier => {
          testWaitersImportFound = LEGACY_TEST_WAITER_FUNCTIONS.includes(specifier.imported.name);
        });

        if (testWaitersImportFound) {
          context.report(node, ERROR_MESSAGE);
        }
      },

      CallExpression(node) {
        if (testWaitersImportFound && LEGACY_TEST_WAITER_FUNCTIONS.includes(node.callee.name)) {
          context.report(node, ERROR_MESSAGE);
        }
      },
    };
