  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        if (routeMap.has(routeInfo.route.fullPath) && !routeMap.has(routeInfo.parent.fullPath)) {
          const existingRouteInfo = routeMap.get(routeInfo.route.fullPath);
          context.report({
            node,
        ...
        routeMap.set(routeInfo.route.fullPath, routeInfo);
      },
    };
  },
};
...
function getParentRoutesPaths(node, stack = []) {
  const parentNode = node.parent;
  if (parentNode) {
    if (ember.isRoute(parentNode)) {
      stack.push(getRouteBasePath(parentNode));
    }
    getParentRoutesPaths(parentNode, stack);
  }
  return stack;
}
