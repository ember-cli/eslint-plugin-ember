  ...
  return (
    ...
    (types.isProperty(node) &&
      ...
      ((types.isMemberExpression(node.value.callee) &&
        ...
        // inject.service()
        (types.isMemberExpression(node.value) &&
          types.isIdentifier(node.value.object.property) &&
          node.value.object.property.name === importedInjectName &&
          types.isIdentifier(node.value.property) &&
          node.value.property.name === 'service')))
  );
}
