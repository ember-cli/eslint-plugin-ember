...
function isEmberModule(callee, element, module) {
  const memberExp = utils.isMemberExpression(callee.object) ? callee.object :
    callee;
...
function isModule(node, element, module) {
  ...
  return utils.isCallExpression(node) &&
    (isLocalModule(node.callee, element) || isEmberModule(node.callee, element,
      moduleName));
}
...
function isEmberCoreModule(node, module, filePath) {
  const isExtended = node.callee.property ? node.callee.property.name ===
    'extend' : false;
  let isModuleByPath;

  if (filePath) {
    isModuleByPath = isModuleByFilePath(filePath, module.toLowerCase()) &&
      isExtended;
  }
...
function isCustomProp(property) {
  ...
  const isCustomObjectProp = utils.isObjectExpression(value) && property.key.name !==
    'actions';
...
function getModuleProperties(module) {
  const firstObjectExpressionNode = utils.findNodes(module.arguments,
    'ObjectExpression')[0];
