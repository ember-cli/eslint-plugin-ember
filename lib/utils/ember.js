...
function isModuleByFilePath(filePath, module) {
  if (filePath.indexOf('app/') === -1) { return false; }

  const fileName = `/${module}.js`;
  const folderName = `${module}s`;
  const possibleModules = ['routes', 'models', 'components', 'controllers'];
  const otherThanCurrentModules = possibleModules.filter(m => m !== folderName);

  if (module === 'model') {
    return filePath.indexOf(`/${folderName}/`) > -1;
  }

  // Check if there are any folders in current path that could match other modules' types
  const notAnyOtherModuleDetected = otherThanCurrentModules.filter(m => filePath.indexOf(`/${m}/`) > -1).length === 0;

  /**
   * Check if current file exist in standard structure,
   * if not check PODS structure,
   * but make sure that not any other module is detected.
   * So when for example filePath is `app/models/route.js` we don't apply routes' rules on it.
   */
  return filePath.indexOf(`/${folderName}/`) > -1 ||
    (filePath.indexOf(fileName) > -1 && notAnyOtherModuleDetected);
}
