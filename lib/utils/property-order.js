...
function determinePropertyType(node, parentType) {
  ...
  if (parentType === 'route') {
    if (ember.isRouteDefaultProp(node)) {
      return 'inherited-property';
    } else if (ember.isBeforeModelProp(node)) {
      return 'beforeModel';
    } else if (ember.isModelProp(node)) {
      return 'model';
    } else if (ember.isAfterModelProp(node)) {
      return 'afterModel';
    } else if (ember.isSerializeProp(node)) {
      return 'serialize';
    } else if (ember.isRedirectProp(node)) {
      return 'redirect';
    } else if (ember.isActivateProp(node)) {
      return 'activate';
    } else if (ember.isSetupControllerProp(node)) {
      return 'setupController';
    } else if (ember.isRenderTemplateProp(node)) {
      return 'renderTemplate';
    } else if (ember.isResetControllerProp(node)) {
      return 'resetController';
    } else if (ember.isDeactivateProp(node)) {
      return 'deactivate';
    }
  }
