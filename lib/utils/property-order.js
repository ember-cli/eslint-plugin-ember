    ...
    if (order < maxOrder) {
      ...
      context.report({
        ...
        fix: (fixer) => {
          ...
          const propertyCommentOffset = getCommentOffsetBefore(property, sourceCode);
          const foundPropertyOffset = getCommentOffsetBefore(foundProperty, sourceCode);

          const offset = nextToken.start - property.end;
          const textBetween = property.start - propertyCommentOffset - foundProperty.end;

          const replaceTextRange = [foundProperty.start - foundPropertyOffset, nextToken.start];
          const movedProperty = sourceCode.getText(property, propertyCommentOffset, offset);
          ...
          const outputText = movedPropertyWithComma + restText;
