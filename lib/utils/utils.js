...
/**
 * Check whether or not a node is a a ThisExpression with name of attrs, which corresponds to a `this.attrs` call.
 *
 * @param {ASTNode} node The node to check.
 * @returns {boolean} Whether or not the node is a ThisExpression with name of attrs.
 */
function isThisAttrsExpression(node) {
  return isThisExpression(node.object) && node.property.name === 'attrs';
}
