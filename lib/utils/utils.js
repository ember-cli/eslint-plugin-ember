 ...
 * already destructured name and checking aliases.
 *
 * @param  {CallExpression} node The node to check.
 * @param  {String}         destructuredName The destructured name.
...
function isGlobalCallExpression(node, destructuredName, aliases) {
  ...
  const isGlobalCall = node.callee && aliases.includes(node.callee.name);
