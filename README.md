...
The `recommended` config covers gjs and gts. It scopes the rules for each with a `files` glob, so a separate config per file type is not necessary. You still need the parser block, because a parser cannot come from a shared config here.
