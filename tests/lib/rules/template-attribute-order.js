...
hbsRuleTester.run('template-attribute-order', rule, {
  valid: [
    ...
    // alphabetize: false allows unordered hash pairs.
    {
      code: '{{MyComponent b="2" a="1"}}',
  ...
  invalid: [
    ...
    // Custom order: attributes before arguments.
    {
      code: '<div @foo="1" {{did-render this.ok}} aria-label="foo"></div>',
      output: null,
      options: [{ order: ['attributes', 'arguments', 'modifiers'] }],
      errors: [{ message: 'Attributes aria-label must go before arguments.' }],
    },
    // Custom order: modifiers first.
    {
      code: '<div @foo="1" {{did-render this.ok}} aria-label="foo"></div>',
      output: null,
      options: [{ order: ['modifiers', 'attributes', 'arguments'] }],
      errors: [{ message: 'Modifiers {{did-render this.ok}} must go before arguments.' }],
    },
    // Hash pair alphabetization (default config).
    {
      code: '{{MyComponent b="2" a="1"}}',
      output: null,
      errors: [{ message: '`a` must appear after `b`.' }],
    },
    // alphabetize: false – only group ordering checked, not alphabetization.
    {
      code: '<MyComponent @value="5" data-test-foo @change={{this.foo}} local-class="foo" {{on "click" this.foo}} ...attributes as |sth|>content</MyComponent>',
      output: null,
      options: [{ alphabetize: false }],
    ...
    // Custom order with curly invocation hash pairs.
    {
      code: '{{my-component one two b=1 a=2}}',
      output: null,
      options: [{ order: ['attributes', 'arguments', 'modifiers'] }],
    ...
    // Explicit alphabetize: true with hash pairs.
    {
      code: '{{MyComponent b="2" a="1"}}',
      output: null,
    ...
    // Shared::Modal with mixed ordering.
    {
      code: '<Shared::Modal b="2" a="1" @close={{action "closeModal"}} as |modal|><p>Text</p></Shared::Modal>',
      output: null,
      errors: [{ message: 'Arguments @close must go before attributes.' }],
    },
  ],
});
